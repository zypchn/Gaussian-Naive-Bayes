...
### Model with Default Hyperparamaters and Raw Data
...
### Model with Default Hyperparamaters and Normalized Data
...
The third and last model performed exactly the same with the former one (both accuracy score and confusion matrix), despite that it was fine-tuned using Grid Search. Amongst the values passed (9e-9, 3e-3, 7e-7, 5e-5), the best estimator had the value of 3e-3. An interesting thing was that models trained with the remaining values performed exactly or very similarly with each other, giving us an array of test score ranking [2, 1, 2, 2]. This could happen due to variety of reasons, which will be discussed in the *Discussion* part. Below is the confusion matrix for this model:
![c6013152-f806-4347-b2b0-402ade373ca1](https://github.com/zypchn/Gaussian-Naive-Bayes/assets/144728809/c9a3cbc9-88ad-453e-bb0a-4df71a1e97a1)


## Discussion
With all accuracy scores compared, we can say that none of our three models are the right one for this dataset. This could mean several things but the highlights -probably- are: either Gaussian Naive Bayes algorithm was not the right choice for this dataset, or the dataset is flawed. And what we call "flaw" could also mean several things: low number instances, missing values, redundant features, etc. In our case the main reason is probably the first one: low number of instances, since diabetes dataset has only 768 instances. But that doesn't mean that other reasons are invalid. For example what we call redundant may be crucial for this classification task, so it is best to get an expert's opinion on this matter. Another odd thing is that when we scaled the training instances, the accuracy score dropped; not significantly but it was still a drop. The reason for that could be that this dataset does not require Min-Max-Scaling since majority of the values are actually not that high. In fact, the highest value in the entire dataset is 846 ("Insulin"). Perhaps a different scaler will perform well. And finally, the stability of accuracy score before and after fine-tuning. We tried four values for the *var_smooting* parameter and one three of four gave us the same scores, hence the ranking array [2, 1, 2, 2]. Perhaps the values passed were not optimal or not that variant for better evaluation, or *var_smoothing* is not a crucial parameter to and default value is sufficant. All the listed possibilities should be considered as a reason for further experimentation on this particular task.
